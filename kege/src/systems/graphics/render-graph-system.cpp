...
#include "render-graph-system.hpp"
#include "render-graph-loader.hpp"

namespace kege{

    RenderGraphSystem::RenderGraphSystem( kege::Engine* engine )
    :   System( engine, "render-graph-system", 0 )
    {}

    bool RenderGraphSystem::initialize()
    {
        uint32_t frames_in_flight = 2;
        Graphics* graphics = _engine->getGraphics();
        _graph = new kege::RenderGraph( graphics );
        ...
        _graph->defineSampler( "sampler-nearest-norep", desc );
        ...
        _graph->defineSampler( "sampler-nearest-rep", desc );
        ...
        desc.address_mode_v = kege::AddressMode::ClampToEdge;
        desc.address_mode_w = kege::AddressMode::ClampToEdge;
        _graph->defineSampler( "sampler-linear-norep", desc );
        ...
        desc.address_mode_v = kege::AddressMode::Repeat;
        desc.address_mode_w = kege::AddressMode::Repeat;
        _graph->defineSampler( "sampler-linear-rep", desc );
        ...
        RgResrcHandle camera_buffer_resource = _graph->defineBuffer
        ({
            .frames_in_flight = frames_in_flight,
            .name = "camera-buffer",
            .info =
        ...
        kege::RgResrcHandle camera_descriptor_resource = _graph->defineShaderResource
        (
            "camera-descriptor",
            frames_in_flight,
            {
        ...
        ({
            ...
            .add_passes = []( kege::RenderGraph* graph )
            {
                ...
                ({
                    .name = "scene-output-to-swapchain",
                    .reads =
                    {
        ...
        setRenderGraph( _graph );
        return System::initialize();
    }

    void RenderGraphSystem::shutdown()
    {
        _graph.clear();
        System::shutdown();
    }
