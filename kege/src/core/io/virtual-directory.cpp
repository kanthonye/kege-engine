...
namespace kege{
    ...
    void VirtualDirectory::add( const kege::string& name, const kege::string& path )
    {
        int32_t directory_index = (int32_t) _directories.size();
        _directories.push_back( path );
        _virmap.insert( name.c_str(), directory_index );
    }

    kege::string VirtualDirectory::fetch( const kege::string& name )
    {
        /*
    ...
    kege::File VirtualDirectory::open( const kege::string& fname )
    {
