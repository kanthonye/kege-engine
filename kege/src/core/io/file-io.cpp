...
namespace kege{
    ...
    kege::VirtualDirectory& FileIO::virdir()
    {
        return *_virdir;
    }
    
    void FileIO::uninitialize()
    {
    }

    bool FileIO::initialize( const kege::string& rootpath )
    {
        ...
        _virdir = new kege::VirtualDirectory();
        std::vector< char > buffer( file.size() + 1 );
        file.read(buffer.data(), 1, file.size());
        buffer[ file.size() ] = 0;
    ...
    kege::Ref< kege::VirtualDirectory > FileIO::_virdir;
