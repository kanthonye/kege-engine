...
//    }
    ...
    float getWidth( Layout& layout, uint32_t pid )
    {
        float width = 0;
        for (uint32_t eid = layout.nodes( pid ).head; eid != 0 ; eid = layout.nodes( eid ).next )
        {
            width += getWidth( layout, eid );
        }

        switch ( layout[ pid ].style.width.type )
        {
            case ui::SizingType::SIZE_FIXED:
            {
                layout[ pid ].rect.width = layout[ pid ].style.width.size;
                break;
            }

            case ui::SizingType::SIZE_PERCENT:
            {
                layout[ pid ].rect.width = layout[ layout.nodes( pid ).parent ].rect.width * layout[ pid ].style.width.size;
    ...
    void alignTopToBottom( Layout& layout, uint32_t pid )
    {
        int count = 0;
        vec2 offset;
        ...
        for (uint32_t eid = layout.nodes( pid ).head; eid != 0 ; eid = layout.nodes( eid ).next )
        {
            if ( layout[ eid ].style.align.self == AlignSelf::Absolute )
            {
            ...
            layout[ eid ].rect.x = offset.x;
            layout[ eid ].rect.y = offset.y;

            offset.y += layout[ eid ].rect.height;
            if( count < layout.nodes( pid ).count)
            {
    ...
    void alignBottomToTop( Layout& layout, uint32_t pid )
    {
        int count = 0;
        vec2 offset;
        offset.x = layout[ pid ].rect.x + layout[ pid ].rect.width + layout[ pid ].style.padding.left;
        offset.y = layout[ pid ].rect.y + layout[ pid ].rect.height + layout[ pid ].style.padding.above;
    ...
    void alignLeftToRight( Layout& layout, uint32_t pid )
    {
        int count = 0;
        vec2 offset;
        offset.x = layout[ pid ].rect.x + layout[ pid ].style.padding.left;
        ...
        for (uint32_t eid = layout.nodes( pid ).head; eid != 0 ; eid = layout.nodes( eid ).next )
        {
            ...
            if( count < layout.nodes( pid ).count )
            {
                offset.x += layout[ pid ].style.gap.width;
            }
            count++;
        }
    }

    void alignRightToLeft( Layout& layout, uint32_t pid )
    {
        int count = 0;
        vec2 offset;
        offset.x = layout[ pid ].rect.x + layout[ pid ].rect.width + layout[ pid ].style.padding.left;
        ...
        for (uint32_t eid = layout.nodes( pid ).head; eid != 0 ; eid = layout.nodes( eid ).next )
        {
            ...
            if( count < layout.nodes( pid ).count - 1 )
            {
                offset.x -= layout[ pid ].style.gap.width;
            }
            count++;
    ...
    float getPercentWidth( Layout& layout, uint32_t eid )
    {
        UIElem& elem = layout[ eid ];
        elem.rect.width = 0;
        if ( 1 <= layout.nodes( eid ).parent )
        {
            uint32_t parent = layout.nodes( eid ).parent;
            elem.rect.width += layout.nodes( parent ).elem.style.padding.left;
            elem.rect.width += layout.nodes( parent ).elem.style.padding.right;
            elem.rect.width += layout[ parent ].style.gap.width * (layout.nodes( parent ).count - 1);
            elem.rect.width += layout[ parent ].rect.width;
        }
        elem.rect.width = elem.rect.width * layout[ eid ].style.width.size;
        return elem.rect.width;
    }

    float getPercentHeight( Layout& layout, uint32_t eid )
    {
        UIElem& elem = layout[ eid ];
        elem.rect.height = 0;
        ...
        {
            uint32_t parent = layout.nodes( eid ).parent;
            elem.rect.height += layout.nodes( parent ).elem.style.padding.above;
            elem.rect.height += layout.nodes( parent ).elem.style.padding.below;
            elem.rect.height += layout[ parent ].style.gap.height * (layout.nodes( parent ).count - 1);
            elem.rect.height += layout[ parent ].rect.height;
        }
        elem.rect.height = elem.rect.height * layout[ eid ].style.height.size;
        return elem.rect.height;
    }

    float getFlexibleWidth( Layout& layout, uint32_t pid )
    {
        UIElem& elem = layout[ pid ];
        elem.rect.width = (elem.style.width.type == ui::SizingType::SIZE_FIXED)
        ...
        if ( 1 <= layout.nodes( pid ).parent )
        {
            elem.rect.width += layout.nodes( layout.nodes( pid ).parent ).elem.style.padding.left;
            elem.rect.width += layout.nodes( layout.nodes( pid ).parent ).elem.style.padding.right;
        }

        for (uint32_t eid = layout.nodes( pid ).head; eid != 0 ; eid = layout.nodes( eid ).next )
        {
            switch ( layout[ eid ].style.width.type )
            {
                case ui::SizingType::SIZE_FIXED:
                {
        ...
        if ( percentage )
        {
            ...
            for (uint32_t eid = layout.nodes( pid ).head; eid != 0 ; eid = layout.nodes( eid ).next )
            {
                if ( layout[ eid ].style.width.type == ui::SizingType::SIZE_PERCENT )
                {
                    width += elem.rect.width * layout[ eid ].style.width.size;
                    count += 1;
                }
            }
        ...
        if ( 2 <= count )
        {
            elem.rect.width += elem.style.gap.width * (count - 1);
        }

        return elem.rect.width;
    }

    float getFlexibleHeight( Layout& layout, uint32_t pid )
    {
        UIElem& elem = layout[ pid ];
        
        float height = (elem.style.height.type == ui::SizingType::SIZE_FIXED)
        ...
        for (uint32_t eid = layout.nodes( pid ).head; eid != 0 ; eid = layout.nodes( eid ).next )
        {
            switch ( layout[ eid ].style.height.type )
            {
                case ui::SizingType::SIZE_FIXED:
                {
                    if ( layout[ pid ].style.align.direction == ui::DIRECTION_LEFT_TO_RIGHT )
                    {
        ...
        if ( percentage )
        {
            ...
            for (uint32_t eid = layout.nodes( pid ).head; eid != 0 ; eid = layout.nodes( eid ).next )
            {
                if ( layout[ eid ].style.height.type == ui::SizingType::SIZE_PERCENT )
                {
                    h += elem.rect.height * layout[ eid ].style.height.size;
                    count += 1;
                }
            }
            height = h;
        ...
        if ( 2 <= count )
        {
            if ( layout[ pid ].style.align.direction == ui::DIRECTION_TOP_TO_BOTTOM || layout[ pid ].style.align.direction == ui::DIRECTION_BOTTOM_TO_TOP )
            {
                height += elem.style.gap.height * (count - 1);
            }
        }

        height += layout[ pid ].style.padding.above;
        height += layout[ pid ].style.padding.below;
        elem.rect.height = height;
        return height;
    }

    float updateChildrenWidth( Layout& layout, uint32_t pid )
    {
        float pad = 0;
        float width = 0;
        ...
        pad += layout.nodes( pid ).elem.style.padding.left;
        pad += layout.nodes( pid ).elem.style.padding.right;
        ...
        if ( percentage )
        {
            ...
            for (uint32_t eid = layout.nodes( pid ).head; eid != 0 ; eid = layout.nodes( eid ).next )
            {
                if ( layout[ eid ].style.width.type == ui::SizingType::SIZE_PERCENT )
                {
                    w += width * layout[ eid ].style.width.size;
                }
            }
            width += w;
        }
        ...
        if ( 0 < extend_count  )
        {
            ...
            if
            (
                layout[ pid ].style.align.direction == DIRECTION_LEFT_TO_RIGHT ||
                layout[ pid ].style.align.direction == DIRECTION_RIGHT_TO_LEFT
            )
            {
                w = w - width - layout[ pid ].style.gap.width * (layout.nodes( pid ).count - 1);
                w = w / float(extend_count);
            }

            for (uint32_t eid = layout.nodes( pid ).head; eid != 0 ; eid = layout.nodes( eid ).next )
            {
                if ( layout[ eid ].style.width.type == ui::SizingType::SIZE_EXTEND )
                {
    ...
    float updateChildrenHeight( Layout& layout, uint32_t pid )
    {
        float pad = 0;
        float height = 0;
        uint32_t extend_count = 0;
        ...
        pad += layout.nodes( pid ).elem.style.padding.above;
        pad += layout.nodes( pid ).elem.style.padding.below;
        ...
        if ( percentage )
        {
            for (uint32_t eid = layout.nodes( pid ).head; eid != 0 ; eid = layout.nodes( eid ).next )
            {
                if ( layout[ eid ].style.height.type == ui::SizingType::SIZE_PERCENT )
                {
                    height += height * layout[ eid ].style.height.size;
                }
            }
        }
        ...
        if ( 0 < extend_count  )
        {
            ...
            if
            (
                layout[ pid ].style.align.direction == DIRECTION_TOP_TO_BOTTOM ||
                layout[ pid ].style.align.direction == DIRECTION_BOTTOM_TO_TOP
            )
            {
                h = h - height - layout[ pid ].style.gap.height * (layout.nodes( pid ).count - 1);
                h = h / float(extend_count);
            }

            for (uint32_t eid = layout.nodes( pid ).head; eid != 0 ; eid = layout.nodes( eid ).next )
            {
                if ( layout[ eid ].style.height.type == ui::SizingType::SIZE_EXTEND )
                {
    ...
    void computeChildrenExtent( Layout& layout, uint32_t pid )
    {
    ...
    void alignChildrenNodes( Layout& layout, uint32_t pid )
    {
        switch ( layout.nodes( pid ).elem.style.align.direction )
        {
