...
//    }
    ...
    CoreGraphics::CoreGraphics( kege::Engine* engine )
    :   CoreSystem< kege::Graphics >( engine, "graphics-system" )
    {}

    bool CoreGraphics::initialize()
    {
        kege::WindowCreateInfo create_window_info = {};
        create_window_info.title = "KEGE";
        create_window_info.width = 1024;
        ...
        string shader_file = _engine->vfs()->fetch( "graphics-shaders/hello-triangle/shader.json" );
        if( !_module->getShaderPipelineManager()->load( "triangle-shader", shader_file.c_str() ) )
        {
            return false;
        }
