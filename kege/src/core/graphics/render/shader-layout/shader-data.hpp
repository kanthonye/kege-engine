...
#include "shader-resrc-binding-set.hpp"
...
namespace kege{
    ...
    public:
        ...
        void setPushConstants( size_t offset, size_t size, const void* data );
        ...
        explicit ShaderData( Ref< const ShaderLayout > layout );
    ...
    private:
        ...
        const ShaderStructField* getField( const ShaderBindingBlock* block, const std::string& field_name )const;

        const ShaderBindingBlock* getBlock( const std::string& block_name )const;

        int8_t* getBufferBlockDataPointer( int index, const ShaderBindingBlock* block );
