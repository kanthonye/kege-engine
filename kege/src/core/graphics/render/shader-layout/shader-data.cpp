...
namespace kege{
    ...
    {
        const ShaderBindingBlock* block = getBlock( block_name );
        if ( block == nullptr )
        {
            return false;
        }
        ...
        {
            int8_t* memptr = getBufferBlockDataPointer( index, block );
            if ( memptr == nullptr ) return false;
    ...
    void ShaderData::setPushConstants( size_t offset, size_t size, const void* data )
    {
    ...
    {
        const Ref< ShaderSetBindingPoint >& binding_point = _layout->shader_set_binding_point_layouts[ set_index ]->binding_points[ binding_index ];
    ...
    bool ShaderData::integrate()
    {
        for ( int set_index = 0 ; set_index < _layout->shader_set_binding_point_layouts.size() ; ++set_index )
        {
            ...
            {
                Ref< ShaderSetBindingPointLayout > ssbl = _layout->shader_set_binding_point_layouts[ set_index ];
    ...
    :   _layout( layout )
    {
        ...
        {
            Ref< ShaderSetBindingPointLayout > ssbl = layout->shader_set_binding_layouts[i];
    ...
    ShaderBindingResrc* ShaderData::getShaderBindingPoint( const std::string& block_name )
    {
        ...
        if ( block == nullptr )
        {
            kege::Log::error << "Undefined ShaderBindingBlock name [ "<< block_name <<" ]" << kege::Log::nl;
            return nullptr;
        }

        if ( block->type != ShaderBindingType::SET )
        {
            kege::Log::error << "Current ShaderBindingBlock name [ "<< block_name <<" ] is not a set." << kege::Log::nl;
            return nullptr;
        }

        ShaderSetBindingPoint* set = (ShaderSetBindingPoint*) block;
    ...
    const ShaderStructField* ShaderData::getField( const ShaderBindingBlock* block, const std::string& field_name )const
    {
        const ShaderStructField* field = block->struct_block->get( field_name );
        if ( field == nullptr )
        {
            kege::Log::error << "ShaderBindingBlock[ " << block->name;
    ...
    const ShaderBindingBlock* ShaderData::getBlock( const std::string& block_name )const
    {
    ...
    int8_t* ShaderData::getBufferBlockDataPointer( int index, const ShaderBindingBlock* block )
    {
        ShaderSetBindingPoint* set = (ShaderSetBindingPoint*) block;
