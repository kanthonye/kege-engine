...
namespace kege::vk{
    ...
    )
    {
        ...
        if ( !graphics_submits.empty() )
        {
            ...
            first_wait_sems.push_back( image_available->vk()->handle );
            ...
            last_signal_sems.push_back( render_complete->vk()->handle );
    ...
    bool QueueManager::present( vk::Swapchain* swapchain, const vk::Semaphore* wait_semaphore )
    {
        if ( _accepting_submissions )
        {
