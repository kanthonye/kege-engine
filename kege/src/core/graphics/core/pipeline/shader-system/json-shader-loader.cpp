...
namespace kege{
    ...
    kege::ArrayType stringToArrayType( const std::string& name )
    {
        static std::map< std::string, kege::ArrayType > types;

        if ( types.empty() )
        {
            types[ "none" ] = kege::ArrayType::None;
            types[ "static" ] = kege::ArrayType::Static;
            types[ "runtime" ] = kege::ArrayType::Runtime;
        }
        ...
        if (!name.empty())
        {
            kege::Log::error << "unsupported ArrayType -> " <<name <<kege::Log::nl;
        }
        return kege::ArrayType::None;
    }
    ...
    {
        kege::RasterizerState state;
        state.fill_mode = json["polygon_mode"]( kege::stringToPolygonMode, kege::FillMode::Fill );
        state.cull_mode = json["cull_mode"]( kege::stringToCullMode, kege::CullMode::None );
        state.front_face = json["front_face"]( kege::stringToFrontFace, kege::FrontFace::CounterClockwise );
        state.depth_bias.enable = json.getBool("depth_bias_enable");
        state.depth_bias.constant_factor = json.getFloat("depth_bias_constant_factor", 0.f);
        state.depth_bias.slope_factor = json.getFloat("depth_bias_slope_factor", 0.f);
        state.depth_bias.clamp = json.getFloat("depth_bias_clamp", 0.f);
        state.depth_clip_enable = json.getBool("depth_clip_enable", true);
        state.scissor_enable = json.getBool("scissor_enable", false);
        state.multisample_enable = json.getBool("multisample_enable", true);
        state.line_width = json.getFloat("line_width", 1.0f);
        return state;
    }
    ...
    kege::JsonShaderContent::Set::Binding parseSetBinding( const kege::Json& json )
    {
        return kege::JsonShaderContent::Set::Binding
        {
            ...
            .count = json.getInt("count"),
            .array_type = kege::stringToArrayType( json.getStr("array_type") ),
        };
    }

    kege::JsonShaderContent::GlobalSet parseGlobalSet( const kege::Json& json )
    {
        return kege::JsonShaderContent::GlobalSet
        {
            .set = json.getInt("set"),
            .binding = json.getInt("index"),
            .semantic = kege::stringToSemantic( json.getStr("semantic") ),
        };
    }
    ...
    kege::JsonShaderContent::PipelineLayout parsePipelineLayout( const kege::Json& json )
    {
        ...
        {
            .global_bindings = json[ "global-bindings" ].toVector( parseGlobalSet ),
...
namespace kege{
    ...
    )
    {
        ...
        for (int shader_index : pipeline_meta.shaders)
        {
            ...
            if( desc.byte_code.empty() )
            {
                ...
                kege::Log::error << "compileGlslToSpv(): failed to compile shader. ->";
        ...
        {
            pipeline_description.pipeline_layout.name = pipeline_meta.name;


            kege::PushBlockDescs& push_blocks = pipeline_description.pipeline_layout.push_block_desc;
            push_blocks.resize( pipeline_layout_meta.blocks.size() );
            uint32_t offset = 0;
            for (int block_index : pipeline_layout_meta.blocks)
            {
                const auto& src = content.push_blocks[ block_index ];
                auto& push_block = push_blocks[ block_index ];

                push_block.id = src.id;
                push_block.name = src.name;
                push_block.count = src.count;
                push_block.size = src.size;
                push_block.stages = src.stages;
                push_block.offset = offset;
                push_block.struct_desc = createShaderStructBlock( src.id, src.fields );
                offset += src.size;
            }


            kege::ShaderPipelineSetDesc& resource_layout_desc = pipeline_description.pipeline_layout.resource_layout_desc;
            resource_layout_desc.resize( pipeline_layout_meta.sets.size() );
            offset = 0;
            for (int set_index : pipeline_layout_meta.sets)
            {
                const auto& src = content.sets[ set_index ];
                auto& set_layout = resource_layout_desc[ set_index ];

                //set_layout. = set.index;
                set_layout.resize( src.bindings.size() );

                for (const auto& b : src.bindings)
                {
                    auto& binding = set_layout[ b.index ];
                    binding.binding_index = b.index;
                    binding.binding_count = b.count;
                    binding.name  = b.id;
                    binding.stages = b.stages;
                    binding.usage = b.usage;
                    binding.array_type = b.array_type;
                    binding.struct_desc = createShaderStructBlock( b.id, b.fields );
                    switch (b.usage) {
        ...
        pipeline_description.dynamic_states.push_back( kege::DynamicState::Viewport );
        pipeline_description.dynamic_states.push_back( kege::DynamicState::Scissor );
        
        kege::ref::ShaderPipeline pipeline = _graphics->createShaderPipeline( pipeline_description );
        if ( pipeline )
        {
            for (int i=0; i<pipeline_layout_meta.global_bindings.size(); ++i)
            {
                int set = pipeline_layout_meta.global_bindings[i].set;
                int binding = pipeline_layout_meta.global_bindings[i].binding;
                kege::GlobalSemantic semantic = pipeline_layout_meta.global_bindings[i].semantic;
                pipeline->insertGlobalBinding( set, binding, semantic );
            }
        }
        return pipeline;
    }
    ...
    ref::ShaderPipeline JsonShaderLoader::load( const std::string& filename )
    {
        ...
        {
            CallbackRequest< kege::GraphicsDevice > request(this, &JsonShaderLoader::operator() );
    ...
    void JsonShaderLoader::operator()( kege::GraphicsDevice* graphics )
    {
        _graphics = graphics;
    }
