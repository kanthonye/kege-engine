...
namespace kege{
    ...
    void parseDepthStencilStates( std::vector< DepthStencil >* depth_stencil_states, kege::Json json )
    {
        for (int i = 0; i < json.count(); ++i)
        {
            ...
            DepthStencil state = {};
            state.depth.enable = elems[ "depth_test" ].toBool( false );
            state.depth.write = elems[ "depth_write" ].toBool( true );
            state.depth.compare_op = stringToCompareOp( elems[ "depth_compare_op" ].toStr( "less" ) );
            state.stencil.reference = elems[ "reference" ].toInt( 0 );

            state.stencil.enable = elems[ "stencil_test_enable" ].toBool( false );
            if ( state.stencil.enable )
            {
                Json front_op = elems[ "front_op" ];
                state.stencil.front.fail_op = stringToStencilOp( front_op[ "fail_op" ].toStr( "keep" ) );
                state.stencil.front.depth_fail_op = stringToStencilOp( front_op[ "depth_fail_op" ].toStr( "keep" ) );
                state.stencil.front.compare_op = stringToCompareOp( front_op[ "compare_op" ].toStr( "always" ) );
                state.stencil.front.compare_mask = front_op[ "compare_mask" ].toInt( 0xFFFFFFFF );
                state.stencil.front.write_mask = front_op[ "write_mask" ].toInt( 0xFFFFFFFF );

                Json back_op = elems[ "back_op" ];
                state.stencil.back.fail_op = stringToStencilOp( back_op[ "fail_op" ].toStr( "keep" ) );
                state.stencil.back.depth_fail_op = stringToStencilOp( back_op[ "depth_fail_op" ].toStr( "keep" ) );
                state.stencil.back.compare_op = stringToCompareOp( back_op[ "compare_op" ].toStr( "keep" ) );
                state.stencil.back.compare_mask = back_op[ "compare_mask" ].toInt( 0xFFFFFFFF );
                state.stencil.back.write_mask = back_op[ "write_mask" ].toInt( 0xFFFFFFFF );
            }
    ...
    BlendState getColorBlendAttachmentState( kege::Json json )
    {
        BlendState state = {};
        if ( json )
        {
            state.src_color_blend = stringToBlendFactor( json[ "src_color_blend" ].toStr( "one" ) );
            state.dst_color_blend = stringToBlendFactor( json[ "dst_color_blend" ].toStr( "zero" ) );
            state.color_blend_op = stringToBlendOp( json[ "color_blend_op" ].toStr( "add" ) );

            state.src_alpha_blend = stringToBlendFactor( json[ "src_alpha_blend" ].toStr( "one" ) );
            state.dst_alpha_blend = stringToBlendFactor( json[ "dst_alpha_blend" ].toStr( "zero" ) );
            state.alpha_blend_op = stringToBlendOp( json[ "alpha_blend_op" ].toStr( "add" ) );

            state.enable = json[ "blend_enable" ].toBool( false );
            state.color_write_mask = ColorWriteMask::All;
        }
        return state;
    }
    ...
    PipelineRendering parsePipelineOutputs( kege::Json json )
    {
        PipelineRendering outputs = {};
        if ( json )
        {
            for (int i = 0; i < json.count(); ++i)
            {
    ...
    void parseColorBlendStates( std::vector< ColorBlendState >* color_blend_states, kege::Json json )
    {
        for (int i = 0; i < json.count(); ++i)
        {
    ...
    void parseRasterizationState( std::vector< RasterizerState >* rasterizer_states, kege::Json json )
    {
        for (int i = 0; i < json.count(); ++i)
        {
            kege::Json elems = json[i];
            ...
            state.fill_mode = json["polygon_mode"]( stringToPolygonMode, kege::FillMode::Fill );
    ...
    void parsePipelineVertexInput( std::vector< VertexLayout >* vertex_layouts, kege::Json json )
    {
        VertexLayout desc;
        if ( json )
        {
...
//
//                kege::VertexAttribute v;
//                v.location = attribute["location"].toInt();
//                v.buffer_index = attribute["binding"].toInt();
...
//    }
    ...
    bool parseShaderStructBlocks( std::vector< Ref<kege::StructBlockDesc> >* uniforms, kege::Json json )
    {
        for (int i = 0; i < json.count(); ++i)
        {
            ...
            Ref<kege::StructBlockDesc> struct_block;
            struct_block->name = uniform.getStr("name");
            //ShaderStructType type = uniform["type"](stringToShaderStructType, {});

            struct_block->members.resize( members.count() );
            int offset = 0;
            for (int k = 0; k < members.count(); ++k)
            {
                Json field = members[k];
                struct_block->members[k].name = field.getStr("name");
                struct_block->members[k].type = field.getStr("type");
                //fields[k].type = stringToShaderVarType( fields[k].stype );
                struct_block->members[k].count = field.getInt("count");
                struct_block->members[k].size = field.getInt("size");

                struct_block->members[k].offset = offset;
                offset += struct_block->members[k].size;
            }

            uniforms->push_back( struct_block );
        }
        return true;
    }
    ...
    (
        kege::GraphicsDevice* graphics, std::vector< ref::Shader >* shaders, kege::Json json, const std::string& path
    )
    {
...
//            }
    ...
    {
        parseShaderStructBlocks( &context->struct_blocks, json[ "data_blocks" ] );
    ...
    )
    {
        ...
        ShaderPipelineDesc create_info = {};
        ...
        if( 0 <= pipeline.rasterizer )    create_info.rasterizer = context.rasterizer_states[ pipeline.rasterizer ];
        if( 0 <= pipeline.vertex_layout ) create_info.vertex_layout = context.vertex_layouts[ pipeline.vertex_layout ];
        create_info.shaders.reserve( pipeline.stages.size() );

        create_info.pipeline_rendering = {};
        create_info.pipeline_rendering.sample_count = SampleCount::Count1;
        for (int i = 0; i<pipeline.outputs.size(); ++i)
        {
            switch ( pipeline.outputs[i].format )
            {
                ...
                case kege::Format::depth_32_stencil_8:
                    create_info.pipeline_rendering.depth_attachment_format = pipeline.outputs[i].format;
                    break;

                default:
                    create_info.pipeline_rendering.color_attachment_formats.push_back({
        ...
        {
//            kege::ShaderPipelineSetDesc& bind_sets = create_info.shader_layout.resource_layout_desc;
//            {
//                bind_sets.resize( pipeline_layout_data.set_layouts.size() );
...
//                    ShaderSetDesc& set = bind_sets[ set_index ];
//
//                    set.resize( set_layout_data.bindings.size() );
//                    //set.index = set_layout_data.set;
//
//                    for (int i = 0; i < set_layout_data.bindings.size(); ++i)
//                    {
//                        set[i].binding_index   = set_layout_data.bindings[i].binding;
//                        set[i].name    = set_layout_data.bindings[i].name;
//                        set[i].binding_count   = set_layout_data.bindings[i].count;
//                        set[i].stages  = set_layout_data.bindings[i].stages;
//                        set[i].usage   = set_layout_data.bindings[i].usage;
//
//                        if ( set_layout_data.bindings[i].block_ref >= 0 )
//                        {
//                            set[i].struct_desc = context.struct_blocks[ set_layout_data.bindings[i].block_ref ];
//                        }
//                    }
//                }
//            }

            create_info.pipeline_layout.push_block_desc.resize( pipeline_layout_data.push_layouts.size() );
            for (int i = 0; i < pipeline_layout_data.push_layouts.size(); ++i)
            {
                PushBlockDesc& block = create_info.pipeline_layout.push_block_desc[i];
                ...
                {
                    block.struct_desc = context.struct_blocks[ pipeline_layout_data.push_layouts[i].block_ref ];
                }
            }
        }
