...
//                break;
//            }
//            case kege::RenderPassType::Geometry:
//            {
...
//                break;
//            }
//            case kege::RenderPassType::Lighting:
//            {
//                description.pipeline_rendering.color_attachment_formats.push_back(kege::Format::rgba_f16);
//                description.color_blend.attachments.push_back( kege::BlendStatePreset::alphaBlend() );
//                description.color_blend.logic_op_enable = true;
...
//        _outputs = {};
    ...
    void ShaderCompiler::writeResource(std::stringstream& source, const std::string& fname)
    {
        ...
        for (const ShaderResource& res : resources)
        {
            ...
            kege::Ref< kege::StructBlockDesc > block;
            if( !res.members.empty() )
            {
                block = new StructBlockDesc;
            ...
            {
                ShaderSetBindingDesc bind_point;
                bind_point.name = res.name;
                bind_point.stages |= _curr_stage->type;
                bind_point.struct_desc = block;
                bind_point.binding_index = binding_index;
                bind_point.binding_count = kege::max<uint32_t>(1, sr.resolve_int(res.count));
                ...
                {
                    ShaderSetDesc& bind_set = _bind_set_descs[ meta->set ];
                    bind_set.push_back(bind_point);
                    //bind_set.name = res.block_name;
                    //bind_set.index = meta->set;
                }
                else
                {
                    m->second.push_back(bind_point);
                }
            }
            else
            {
    ...
    ShaderCompiler::ShaderCompiler(kege::GraphicsDevice* graphics, const std::string& filepath)
    :   _graphics(graphics)
    ,   _filepath(filepath)
    {}
}
