...
namespace kege::ref
{
    typedef kege::Ref< kege::CommandEncoder > CommandEncoder;
}

namespace kege::cref
{
    typedef kege::Ref< const kege::CommandEncoder > CommandEncoder;
}
...
namespace kege{
    ...
    struct BufferMemoryBarrier
    {
        ...
        /// @{
        kege::AccessFlags src_access;     ///< Access type before the barrier (e.g., what operations were done)
        kege::AccessFlags dst_access;     ///< Access type after the barrier (e.g., what operations will be done)
        /// @}

        /// @name Pipeline Synchronization
        /// @{
        kege::PipelineStageFlag src_stage;   ///< Pipeline stage where previous accesses completed
        kege::PipelineStageFlag dst_stage;   ///< Pipeline stage where subsequent accesses will begin
        /// @}
