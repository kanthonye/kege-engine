...
namespace kege::vk{
    ...
    Image::Image( vk::Device* device, const kege::ImageDesc& desc )
    :   kege::Image( desc )
    ...
    ,   _device( device )
    {
        ...
        if ( device->getVkInstance()->isValidationEnabled() && !desc.name.empty() )
        {
            device->core().debugSetObjectName( (uint64_t)_memory, VK_OBJECT_TYPE_DEVICE_MEMORY, desc.name.c_str() );
            device->core().debugSetObjectName( (uint64_t)_view, VK_OBJECT_TYPE_IMAGE_VIEW, desc.name.c_str() );
            device->core().debugSetObjectName( (uint64_t)_image, VK_OBJECT_TYPE_IMAGE, desc.name.c_str() );
        }
