...
namespace kege::vk{
    ...
    GraphicsDevice* Instance::createDevice( kege::PhysicalDevice* physical_device )
    {
        ...
        {
            kege::Log::error << "Device initalization failed in Instance::createDevice()." <<Log::nl;
            device->shutdown();
...
//            ref.clear();
    ...
    bool Instance::initalize( const DeviceInitializationInfo& info )
    {
        if ( createInstance( info ) )
        {
            ...
            if ( device_count == 0 )
            {
                kege::Log::error << "Failed to find GPUs with Vulkan support!" <<Log::nl;
                return false;
            }
    ...
    PhysicalDevice* Instance::getBestSuitablePhysicalDevice( const DeviceInitializationInfo& info )
    {
        ...
        GLFWwindow* window_handle = (GLFWwindow*) info.window->getHandle();
        VkResult result = glfwCreateWindowSurface( _instance, window_handle, nullptr, &_surface );
...
//                         << info.min_dedicated_video_memory << "-GB required).\n";
    ...
    bool Instance::checkRayTracingSupport( VkPhysicalDevice device )
    {
        ...
        {
            kege::Log::error << "    - Missing feature: accelerationStructure\n";
            hasSupport = false;
        }
        if (!pipeline_features.rayTracingPipeline)
        {
            kege::Log::error << "    - Missing feature: rayTracingPipeline\n";
            hasSupport = false;
        }
    ...
    bool Instance::createInstance( const DeviceInitializationInfo& info )
    {
        ...
        {
            kege::Log::error << "Failed to create Vulkan instance!" <<Log::nl;
            return false;
        }
    ...
    {
        for ( vk::Device* device = _devices.head; device != nullptr; device = device->next )
            device->shutdown();
        _devices.clear();
    ...
    )
    {
        switch ( message_severity )
        {
            ...
            case VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT:
            {
                kege::Log::error <<"[ PERFORMANCE ] : " << callback_data->pMessage <<"\n";
                break;
            }
