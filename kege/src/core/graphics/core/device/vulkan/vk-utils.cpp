...
namespace kege::vk{
    ...
    VkBufferUsageFlags convertBufferUsageFlags( BufferUsages flags )
    {
        ...
        if (( flags & BufferUsages::CopySrc ) == BufferUsages::CopySrc )
        {
            usage |= VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
        }
        if (( flags & BufferUsages::CopyDst ) == BufferUsages::CopyDst )
        {
            usage |= VK_BUFFER_USAGE_TRANSFER_DST_BIT;
        }
        if (( flags & BufferUsages::VertexBuffer ) == BufferUsages::VertexBuffer )
        {
            usage |= VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
        }
        else if (( flags & BufferUsages::IndexBuffer ) == BufferUsages::IndexBuffer )
        {
            usage |= VK_BUFFER_USAGE_INDEX_BUFFER_BIT;
        }
        else if (( flags & BufferUsages::UniformBuffer ) == BufferUsages::UniformBuffer )
        {
            usage |= VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
        }
        else if (( flags & BufferUsages::StorageBuffer ) == BufferUsages::StorageBuffer )
        {
            usage |= VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;
        }
        else if (( flags & BufferUsages::IndirectBuffer ) == BufferUsages::IndirectBuffer )
        {
            usage |= VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT;
        }
        else if (( flags & BufferUsages::UniformTexelBuffer ) == BufferUsages::UniformTexelBuffer )
        {
            usage |= VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT;
        }
        else if (( flags & BufferUsages::StorageTexelBuffer ) == BufferUsages::StorageTexelBuffer )
        {
    ...
    VkCullModeFlags convertCullMode( CullMode mode )
    {
        switch ( mode )
        {
            ...
            default: kege::Log::error << "Unsupported CullMode in convertCullMode()" <<Log::nl;
        }
    }
    ...
    VkFrontFace convertFrontFace( FrontFace face )
    {
        switch ( face )
        {
            ...
            default: kege::Log::error << "Unsupported FrontFace in convertFrontFace()" <<Log::nl;
        }
    }
    ...
    VkImageViewType convertTextureViewType(ImageType type)
    {
        switch ( type )
        {
            ...
            default: kege::Log::error << "UNKNOW ImageType in convertTextureViewType()" <<Log::nl;
                break;
        }
    }
    ...
    VkBufferUsageFlags convertBufferUsage(BufferUsages usage)
    {
        ...
        if ((usage & BufferUsages::CopySrc) != BufferUsages::None) {
            vk_usage |= VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
        }
        if ((usage & BufferUsages::CopyDst) != BufferUsages::None) {
            vk_usage |= VK_BUFFER_USAGE_TRANSFER_DST_BIT;
        }
        if ((usage & BufferUsages::VertexBuffer) != BufferUsages::None) {
            vk_usage |= VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
        }
        if ((usage & BufferUsages::IndexBuffer) != BufferUsages::None) {
            vk_usage |= VK_BUFFER_USAGE_INDEX_BUFFER_BIT;
        }
        if ((usage & BufferUsages::UniformBuffer) != BufferUsages::None) {
            vk_usage |= VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
        }
        if ((usage & BufferUsages::StorageBuffer) != BufferUsages::None) {
        ...
        if ((usage & BufferUsages::IndirectBuffer) != BufferUsages::None) {
            vk_usage |= VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT;
        }
        if ((usage & BufferUsages::ShaderDeviceAddress) != BufferUsages::None) {
        ...
        // if ((usage & BufferUsages::UniformTexelBuffer) != BufferUsages::None) {
        //     vkUsage |= VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT;
        // }
        // if ((usage & BufferUsages::StorageTexelBuffer) != BufferUsages::None) {
...
//    }
    ...
    VkShaderStageFlagBits convertShaderStage(ShaderStageFlag stage)
    {
        switch (stage)
        {
            case ShaderStageFlag::Vertex:  return VK_SHADER_STAGE_VERTEX_BIT;
            case ShaderStageFlag::Fragment:  return VK_SHADER_STAGE_FRAGMENT_BIT;
            case ShaderStageFlag::Compute:  return VK_SHADER_STAGE_COMPUTE_BIT;
            case ShaderStageFlag::Geometry:  return VK_SHADER_STAGE_GEOMETRY_BIT;
            case ShaderStageFlag::TessellationControl: return VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT;
            case ShaderStageFlag::TessellationEvaluation: return VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT;
            default:
                // Handle invalid input, perhaps default to 1 or assert/log error
                kege::Log::error << "Invalid shader stage encountered." <<Log::nl;
                return {};
        }
    }
    ...
    VkShaderStageFlags convertShaderStageMask(ShaderStageFlag stage)
    {
        ...
        if ((stage & ShaderStageFlag::All) == ShaderStageFlag::All)
        {
            vkFlags |= VK_SHADER_STAGE_ALL;
        }
        else if ((stage & ShaderStageFlag::AllGraphics) == ShaderStageFlag::AllGraphics)
        {
            vkFlags |= VK_SHADER_STAGE_ALL_GRAPHICS;
        }
        else
        {
            if ((stage & ShaderStageFlag::Vertex) != ShaderStageFlag::Invalid) { // Use static_cast for type safety with None=0
                vkFlags |= VK_SHADER_STAGE_VERTEX_BIT;
            }
            if ((stage & ShaderStageFlag::Fragment) != ShaderStageFlag::Invalid) {
                vkFlags |= VK_SHADER_STAGE_FRAGMENT_BIT;
            }
            if ((stage & ShaderStageFlag::Geometry) != ShaderStageFlag::Invalid) {
                vkFlags |= VK_SHADER_STAGE_GEOMETRY_BIT;
            }
            if ((stage & ShaderStageFlag::TessellationControl) != ShaderStageFlag::Invalid) {
                vkFlags |= VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT;
            }
            if ((stage & ShaderStageFlag::TessellationEvaluation) != ShaderStageFlag::Invalid) {
                vkFlags |= VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT;
            }
            if ((stage & ShaderStageFlag::Compute) != ShaderStageFlag::Invalid) {
    ...
    VkFilter convertFilter(Filter filter)
    {
        switch ( filter )
        {
            ...
            default: kege::Log::error << "Unknown Filter in convertFilter()" <<Log::nl;
                break;
        }
    }
    ...
    VkSamplerMipmapMode convertMipmapMode(MipmapMode filter)
    {
        switch ( filter )
        {
            ...
            default: kege::Log::error << "Unknown Filter in convertMipmapMode()" <<Log::nl;
                break;
        }
    }
    ...
    VkSamplerAddressMode convertAddressMode(AddressMode mode)
    {
        switch ( mode )
        {
            ...
            default: kege::Log::error << "Unknown AddressMode in convertAddressMode()" <<Log::nl;
                break;
        }
    }
    ...
    VkStencilOp convertStencilOp(StencilOp op)
    {
        switch ( op )
        {
            ...
            default: kege::Log::error << "Unknown StencilOp in convertStencilOp()" <<Log::nl;
                break;
        }
    }
    ...
    VkBorderColor convertBorderColor(BorderColor color)
    {
        switch ( color )
        {
            ...
            default: kege::Log::error << "Unknown BorderColor in convertBorderColor()" <<Log::nl;
                break;
        }
    }
    ...
    VkImageLayout toImageLayout( ImageLayout image_layout )
    {
        switch ( image_layout )
        {
    ...
    const char* vkResultToString(VkResult result)
    {
        switch (result)
        {
            ...
            case VK_ERROR_INVALID_OPAQUE_CAPTURE_ADDRESS:
                return "VK_ERROR_INVALID_OPAQUE_CAPTURE_ADDRESS: A buffer creation or memory allocation failed because the requested address is not available";
            ...
            case VK_ERROR_VIDEO_PROFILE_OPERATION_NOT_SUPPORTED_KHR:
                return "VK_ERROR_VIDEO_PROFILE_OPERATION_NOT_SUPPORTED_KHR: A video profile operation is not supported";
            case VK_ERROR_VIDEO_PROFILE_FORMAT_NOT_SUPPORTED_KHR:
                return "VK_ERROR_VIDEO_PROFILE_FORMAT_NOT_SUPPORTED_KHR: A video profile format is not supported";
            case VK_ERROR_VIDEO_PROFILE_CODEC_NOT_SUPPORTED_KHR:
                return "VK_ERROR_VIDEO_PROFILE_CODEC_NOT_SUPPORTED_KHR: A video profile codec is not supported";
            case VK_ERROR_VIDEO_STD_VERSION_NOT_SUPPORTED_KHR:
                return "VK_ERROR_VIDEO_STD_VERSION_NOT_SUPPORTED_KHR: A video standard version is not supported";
            case VK_ERROR_INVALID_DRM_FORMAT_MODIFIER_PLANE_LAYOUT_EXT:
                return "VK_ERROR_INVALID_DRM_FORMAT_MODIFIER_PLANE_LAYOUT_EXT: Invalid DRM format modifier plane layout";
            case VK_ERROR_NOT_PERMITTED_KHR:
                return "VK_ERROR_NOT_PERMITTED_KHR: An operation is not permitted";
            case VK_ERROR_FULL_SCREEN_EXCLUSIVE_MODE_LOST_EXT:
                return "VK_ERROR_FULL_SCREEN_EXCLUSIVE_MODE_LOST_EXT: An operation caused a full-screen exclusive mode to be lost";
            ...
            case VK_OPERATION_DEFERRED_KHR:
                return "VK_OPERATION_DEFERRED_KHR: An operation was deferred to a later time";
            case VK_OPERATION_NOT_DEFERRED_KHR:
                return "VK_OPERATION_NOT_DEFERRED_KHR: An operation was not deferred to a later time";
            case VK_PIPELINE_COMPILE_REQUIRED_EXT:
                return "VK_PIPELINE_COMPILE_REQUIRED_EXT: A pipeline compile is required";
            default:
