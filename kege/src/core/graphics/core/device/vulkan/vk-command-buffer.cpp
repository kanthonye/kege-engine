...
namespace kege::vk{
    ...
    kege::Semaphore* CommandBuffer::getSemaphore()
    {
        return _semaphore.ref();
    }

    bool CommandBuffer::bind( uint32_t global_uniform_key, const VkDescriptorSet& set )
    {
        ...
        uint32_t set_index = _bind_pipeline_layout->getBindLocationIndex( global_uniform_key );

        vkCmdBindDescriptorSets
        (
            _handle,
            _current_pipeline_bindpoint,
            _bind_pipeline_layout->handle(),
            set_index,
            1, &set,
            0, nullptr
        );
    ...
    void CommandBuffer::bindShaderResourceSet( const kege::ShaderResourceSet* set )
    {
        bind( set->getUniformKey(), set->vk()->handle() );
    }
//    bool CommandBuffer::bind( size_t count, const kege::vk::ShaderResourceSet* sets )
//    {
...
//    bool CommandBuffer::bind( const kege::vk::ShaderResourceSet& set )
//    {
...
//        }
//
//        uint32_t set_index = _bind_pipeline_layout->getBindLocationIndex( set.getUniformKey() );
//
//        vkCmdBindDescriptorSets
//        (
//            _handle,
...
//    {
//        return false;/// bind( indexed_set.index, indexed_set.set->vk()->handle() );
//    }
...
//    }
    ...
    void CommandBuffer::setPushBlock( ShaderStageFlag stages, uint32_t offset, uint32_t size, const void *data )
    {
        ...
        vkCmdPushConstants( _handle, _bind_pipeline_layout->handle(), stage_flags, offset, size, data );
    }

    void CommandBuffer::bindShaderPipeline( const ref::ShaderPipeline& shader_pipeline )
    {
        _bind_pipeline = shader_pipeline->vk();
        if ( !_bind_pipeline )
        {
        ...
        vkCmdBindPipeline( _handle, VK_PIPELINE_BIND_POINT_GRAPHICS, _bind_pipeline->handle() );

        if ( _bind_pipeline->getType() == kege::PipelineType::Graphics ) {
            _current_pipeline_bindpoint = VK_PIPELINE_BIND_POINT_GRAPHICS;
        }
        else if ( _bind_pipeline->getType() == kege::PipelineType::Compute ) {
            _current_pipeline_bindpoint = VK_PIPELINE_BIND_POINT_COMPUTE;
        }
        _bind_pipeline_layout = _bind_pipeline->getShaderPipelineLayout()->vk();
    }
...
//        vk::Image* image = _device->_textures.get(image.id);
        ...
        barrier.oldLayout = vk::toImageLayout(old_layout);
        barrier.newLayout = vk::toImageLayout(new_layout);
    ...
    void vk::CommandBuffer::beginRendering(const RenderingInfo& rendering_info)
    {
        ...
        for (const RenderingAttachmentInfo& attach_info : rendering_info.color_attachments)
        {
            ...
            color_attachment.imageLayout = toImageLayout( attach_info.image_layout );
        ...
        if ( rendering_info.depth_attachment.has_value() )
        {
            ...
            if (depth_image)
            {
                ...
                depth_attachment.imageLayout = toImageLayout( attach_info.image_layout );// stateToVkLayout(attach_info.initial_layout, depth_image->desc.format);
        ...
        if ( rendering_info.stencil_attachment.has_value() )
        {
            ...
            if ( stencil_image )
            {
                ...
                stencil_attachment.imageLayout = toImageLayout( attach_info.image_layout );// stateToVkLayout(attach_info.initial_layout, stencil_image->desc.format);
    ...
    )
    {
        ...
        {
            const VkBuffer buffer_handle = buffers[i]->vk()->getHandle();
    ...
    void CommandBuffer::bindIndexBuffer(const ref::Buffer& buffer, uint64_t offset, bool use_uint16)
    {
        ...
        vkCmdBindIndexBuffer( _handle, b->getHandle(), static_cast<VkDeviceSize>(offset), index_type);
    }
    ...
    void vk::CommandBuffer::setScissor(const Scissor& scissor)
    {
        if (!_is_recording || !_handle) return;
        VkRect2D vk_scissor = {};
    ...
    void CommandBuffer::drawIndexIndirect( const ref::Buffer& buffer, VkDeviceSize offset, uint32_t draw_count, uint32_t stride )
    {
        const vk::Buffer* b = buffer->vk();
//        if ( _sets_bind_state != _bind_pipeline_layout->getSetLayoutBindSignature() )
//        {
//            kege::Log::error
//                << "[DRAW ERROR] Descriptor set bind mismatch: "
//                << "Pipeline layout expects signature 0x" << std::bitset<8>(_bind_pipeline_layout->getSetLayoutBindSignature()).to_string()
...
//        }
        _sets_bind_state = 0;
        vkCmdDrawIndexedIndirect( _handle, b->getHandle(), offset, draw_count, stride );
    }

    void CommandBuffer::drawIndirect( const ref::Buffer& buffer, VkDeviceSize offset, uint32_t draw_count, uint32_t stride )
    {
        const vk::Buffer* b = buffer->vk();
//        if ( _sets_bind_state != _bind_pipeline_layout->getSetLayoutBindSignature() )
//        {
...
//        }
        _sets_bind_state = 0;
        vkCmdDrawIndirect( _handle, b->getHandle(), offset, draw_count, stride );
    }
    ...
    )
    {
        ...
        for ( ImageMemoryBarrier& barrier : image_barriers )
        {
            ...
            img_barrier.oldLayout = toImageLayout( barrier.old_layout );
            img_barrier.newLayout = toImageLayout( barrier.new_layout );
        ...
        for ( BufferMemoryBarrier& barrier : buffer_barriers )
        {
            ...
            const VkBuffer buffer = barrier.buffer->vk()->getHandle();
...
//    {}
    ...
    vk::CommandBuffer::CommandBuffer(Device* device, kege::QueueType type, VkCommandPool command_pool)
    :   _device( device )
    ...
    ,   _sets_bind_state( 0 )
    {
        ...
        if ( vkAllocateCommandBuffers( _device->handle(), &info, &_handle ) != VK_SUCCESS )
        {
            kege::Log::error << "Something went wrong while creating command buffer!"<<Log::nl;
            return;
        }
