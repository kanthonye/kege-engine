...
namespace kege::vk{
    ...
    const ref::Semaphore& Swapchain::getFrameAvailableSemaphore()const
    {
        return _gpu_frame_syncs[ _image_index ].image_available_semaphore;
    }

    const ref::Semaphore& Swapchain::getFrameFinishSemaphore()const
    {
        return _gpu_frame_syncs[ _image_index ].render_complete_semaphore;
    }
    ...
    GpuFrameSync* Swapchain::getGpuFrameSync()
    {
        return &_gpu_frame_syncs[ _image_index ];
    }

    kege::Result Swapchain::acquireNextImage()
    {
        ...
        (
            ...
            _swapchain,
            UINT64_MAX,
            _gpu_frame_syncs[ _frame_index ].image_available_semaphore->vk()->handle,
            VK_NULL_HANDLE,
            &_image_index
        );
        ...
        {
            kege::Log::error << "Failed to acquire swapchain image! vkResult=" << vkResultToString(result) << Log::nl;
            return kege::Result::UNKNOWN_ERROR;
        }

        _frame_index = (_frame_index + 1) % (uint32_t)_frames.size();
    ...
    kege::Result Swapchain::recreate()
    {
        ...
        destroyFrameData(_frames);

        if ( createFrameData( _frames )!= kege::Result::SUCCESS )
        {
            return kege::Result::FAILED_CREATING_OBJECT;
        }

        _image_index = 0; // CRITICAL: Reset indices after recreation
    ...
    kege::Result Swapchain::createFrameData( kege::array< vk::FrameData >& frames )
    {
        ...
        for (uint32_t i = 0; i < image_count; ++i)
        {
            ...
            _frames[i].color->operator()(kege::ImageDesc{
        ...
        _gpu_frame_syncs.resize( image_count );
        for (int i=0; i<_frames.size(); ++i)
        {
            _gpu_frame_syncs[i].image_available_semaphore = _device->createSemaphore();
            if( _gpu_frame_syncs[i].image_available_semaphore == VK_NULL_HANDLE )
            {
                destroy();
                return kege::Result::FAILED_CREATING_OBJECT;
            }

            _gpu_frame_syncs[i].render_complete_semaphore = _device->createSemaphore();
            if( _gpu_frame_syncs[i].render_complete_semaphore == VK_NULL_HANDLE )
            {
                destroy();
                return kege::Result::FAILED_CREATING_OBJECT;
            }
    ...
    void Swapchain::destroyFrameData( kege::array< vk::FrameData >& frames )
    {
        for (int i=0; i<frames.size(); ++i)
        {
            _device->destroyImage(frames[i].color->vk());
            _device->destroyImage(frames[i].depth->vk());

            _device->destroySemaphore(_gpu_frame_syncs[i].image_available_semaphore->vk());
            _device->destroySemaphore(_gpu_frame_syncs[i].render_complete_semaphore->vk());
        }
        _gpu_frame_syncs.clear();
        frames.clear();
    }
