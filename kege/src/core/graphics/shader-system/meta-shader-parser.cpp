...
#define CONSUME(tok) this->consume(tok, __FUNCTION__)
    ...
    void Parser::parseSets(AST& ast)
    {
        ...
        while (_current_token.type != TokenType::RBRACE && _current_token.type != TokenType::END_OF_FILE)
        {
            if (match(TokenType::AT))
            {
                ...
                {
                    ast.pipeline_description->shader_layout.bind_sets.push_back( parseSet() );
                }
                else
                {
    ...
    BindSetDesc Parser::parseSet()
    {
        ...
        while (_current_token.type != TokenType::RBRACE && _current_token.type != TokenType::END_OF_FILE)
        {
            if (match(TokenType::AT))
            {
                ...
                {
                    set.bindings.push_back( parseBinding() );
                }
    ...
    BindPointDesc Parser::parseBinding()
    {
        BindPointDesc binding;

        CONSUME(TokenType::KW_BINDING);
        CONSUME(TokenType::LPAREN);
        binding.index = parseInt();
        CONSUME(TokenType::RPAREN);
        Properties params = parseEnclosedBracesParameters();
        ...
        if (it != params.end()) {
            binding.count = std::stoi(it->second[0]);
        }
    ...
    void Parser::parsePushBlocks(AST& ast)
    {
        ...
        while (_current_token.type != TokenType::RBRACE && _current_token.type != TokenType::END_OF_FILE)
        {
            if (match(TokenType::AT))
            {
                ...
                {
                    ast.pipeline_description->shader_layout.push_blocks.push_back( parsePushRange() );
                }
                else EXPECTED(TokenType::KW_BINDING, "Expected 'binding' after @");
            }
            else
    ...
    void Parser::parseDynamicState(AST& ast)
    {
        ...
        while (_current_token.type != TokenType::RBRACE && _current_token.type != TokenType::END_OF_FILE)
        {
            ast.pipeline_description->dynamic_states |= stringToDynamicState(_current_token.lexeme);
            advance();
        }
        expect(TokenType::RBRACE, "Expected '}'", __FUNCTION__);
    }
