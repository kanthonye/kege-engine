...
namespace kege::ui{
    ...
    void Layout::resize( uint32_t max_nodes )
    {
        ...
        {
            _id_pool[i].index = i + 1;
        }
        _id_pool[ max_nodes - 1 ].index = -1; // Last node is the end.
        _id_head = 1; // The first available ID is 0.
    }
...
//    }
    ...
    {
        if ( index < 0 || index >= _id_pool.size()) return; // Invalid ID
