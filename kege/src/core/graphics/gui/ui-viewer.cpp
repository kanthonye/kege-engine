...
namespace kege::ui{
    ...
    void Viewer::view( kege::CommandBuffer* command, ui::Layout& layout )
    {
        CommandEncoder* encoder = command->createCommandEncoder();
    ...
    bool Viewer::initialize( Graphics* graphics )
    {
        ...
        _pipeline = _graphics->getShaderPipelineManager()->load( "ui-shader", "/Users/kae/Developer/xcode/KE-GE/kege/assets/shaders/glsl/graphics/gui/gui-rounded-corner-sdf-text.json" );
        if ( !_pipeline )
        {
            return 0;
        }

        _font = ui::FontCreator::create( _graphics, 8, 16, "/Users/kae/Developer/xcode/KE-GE/kege/assets/fonts/monaco.tga" );
        if ( !_font )
        {
            return 0;
        }
...
//         });
    ...
    void Viewer::shutdow()
    {
        if ( _graphics )
        {
            ...
            for (int i = 0; i<kege::MAX_FRAMES_IN_FLIGHT; ++i) _graphics->freeDescriptorSet( _shader_resource_draw_buffer[i] );
