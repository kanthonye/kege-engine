...
//        }
    ...
    void FlatTerrainRenderer::submit( const FlatTerrainPatch& patch )
    {
        ...
        int total_indices = _index_draw_commands[ patch.index_buffer_id ].index_count;

        _batchs[ _curr_batch ].params[ index ].first_instance = index;
        _batchs[ _curr_batch ].params[ index ].instance_count = 1;
        _batchs[ _curr_batch ].params[ index ].vertex_count = total_indices;
...
//        _batchs.clear();
    ...
    FlatTerrainRenderer::FlatTerrainRenderer( kege::GraphicsDevice* graphics )
    :   _settings( nullptr )
    ,   _graphics( graphics )
    ,   _encoder( nullptr )
    ,   _curr_batch( 0 )
